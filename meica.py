...
headsl.append('#'+" ".join(sys.argv).replace(options.dsinputs,r'"%s"' % options.dsinputs ).replace('"',r'"'))
...
if not options.resume and not options.tedica_only and not options.select_only:
...
sl.append("%s%s %s %s -e %s  -d %s --sourceTEs=%s --kdaw=%s --rdaw=1 --initcost=%s --finalcost=%s --conv=2.5e-5 %s" % (tedflag,sys.executable,interactive_flag, '/'.join([meicadir,tedanapath]),options.tes,ica_input,options.sourceTEs,options.daw,options.initcost,options.finalcost,options.ted_args))
sl.append("#")
if outprefix=='': outprefix=setname

if options.select_only: 
	sl = []
	sl.append("%s %s %s -e %s  -d %s --sourceTEs=%s --kdaw=%s --rdaw=1 --initcost=%s --finalcost=%s --conv=2.5e-5 --mix=meica_mix.1D %s " % (sys.executable,interactive_flag, '/'.join([meicadir,tedanapath]),options.tes,ica_input,options.sourceTEs,options.daw,options.initcost,options.finalcost,options.ted_args))
...
sl.append("%scp TED/hik_ts_OC.nii TED/%s_hikts.nii" % (tedflag,outprefix))
sl.append("%scp TED/feats_OC2.nii TED/%s_mefcz.nii" % (tedflag,outprefix))
sl.append("%scp TED/comp_table.txt %s/%s_ctab.txt" % (tedflag,startdir,outprefix))
sl.append("%scp TED/meica_mix.1D %s/%s_mmix.1D" % (tedflag,startdir,outprefix))
hist_line = "%s" % (" ".join(sys.argv).replace('"',r"\""))
note_line = "Denoised timeseries (no thermal noise), produced by ME-ICA v3.0"
sl.append("%s3dNotes -h \'%s (%s)\' TED/%s_hikts.nii" % (tedflag,hist_line,note_line,outprefix))
note_line = "Denoised timeseries (including thermal noise), produced by ME-ICA v3.0"
sl.append("%s3dNotes -h \'%s (%s)\' TED/%s_medn.nii" % (tedflag,hist_line,note_line,outprefix))
note_line = "Z-normalized spatial component maps, produced by ME-ICA v3.0"
sl.append("%s3dNotes -h \'%s (%s)\' TED/%s_mefcz.nii" % (tedflag,hist_line,note_line,outprefix))
note_line = "Denoised ICA coeff. set for ME-ICR seed-based FC analysis, produced by ME-ICA v3.0"
sl.append("%s3dNotes -h \'%s (%s)\' TED/%s_mefc.nii" % (tedflag,hist_line,note_line,outprefix))
note_line = "Full ICA coeff. set for component assessment, produced by ME-ICA v3.0"
sl.append("%s3dNotes -h \'%s (%s)\' TED/%s_mefl.nii" % (tedflag,hist_line,note_line,outprefix))
note_line = "T2* weighted average of ME time series, produced by ME-ICA v3.0"
sl.append("%s3dNotes -h \'%s (%s)\' TED/%s_tsoc.nii" % (tedflag,hist_line,note_line,outprefix))
if options.anat!='' and options.space!=False:
	sl.append("%snifti_tool -mod_hdr -mod_field sform_code 2 -mod_field qform_code 2 -infiles TED/%s_hikts.nii -overwrite" % (tedflag,outprefix))
	...
	sl.append("%snifti_tool -mod_hdr -mod_field sform_code 2 -mod_field qform_code 2 -infiles TED/%s_mefcz.nii -overwrite" % (tedflag,outprefix))
sl.append("%sgzip -f TED/%s_medn.nii TED/%s_mefc.nii TED/%s_tsoc.nii TED/%s_mefl.nii TED/%s_mefcz.nii TED/%s_hikts.nii" % (tedflag,outprefix,outprefix,outprefix,outprefix,outprefix,outprefix))
sl.append("%smv TED/%s_medn.nii.gz TED/%s_mefc.nii.gz TED/%s_tsoc.nii.gz TED/%s_mefl.nii.gz TED/%s_hikts.nii.gz TED/%s_mefcz.nii.gz %s" % (tedflag,outprefix,outprefix,outprefix,outprefix,outprefix,outprefix,startdir))
