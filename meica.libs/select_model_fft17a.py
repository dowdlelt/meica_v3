...
def selcomps(seldict,debug=False,olevel=2,oversion=99,knobargs='',filecsdata=False,savecsdiag=True):
	...
	for ii in nc:
		...
		try: tt_table[ii,:2] = stats.ttest_ind(signal_FR2_Z,noise_FR2_Z,equal_var=True)
		except: pass
	...
	fw4m = []
	for ii in nc: 
		fproj = np.fft.fftshift(np.abs(np.fft.rfftn(unmask(seldict['tsoc_B'],mask)[:,:,:,ii]))).max(2).flatten()
		fw4m.append(np.array(fproj>fproj.max()/4,dtype=np.int).sum())
		fproj_arr[:,ii] = rankvec(fproj)
		fproj_arr_val[:,ii] = fproj
	spr = np.array(fw4m)
	...
	spz2 = (fproj_sel_v2-fproj_sel_v2[group0].mean())/fproj_sel_v2[group0].std()
	#Temporal features
	...
	"""
	#Update features
	Tz2 = (tt_table[:,0]-tt_table[group0,0].mean())/tt_table[group0,0].std()
	...
	fz = np.array([spz2,Tz2])		
	midk_clf,clf_ = do_svm(fz[:,np.union1d(group0,rej)].T,[0]*len(group0) + [1]*len(rej),fz[:,to_clf].T,svmtype=1)
	...
	to_ign = []
	#Distinguish disturbances to discard
	minK_ign = np.max([F05,getelbow2(Kappas,True)])  #Preventing exclusion of low variance tail components
	...
	ign_cand = np.unique(list(field_art)+list(phys_art))
	...
	if savecsdiag:
		diagstepkeys=['selmodelversion','rej','KRcut','Kcut','Rcut','dbscanfailed','KRguess','group0','dice_rej','rej_supp','to_clf','midk','toacc_hi','toacc_lo','field_art','phys_art','ncl','ign']
		diagstepout=[]
		for ddk in diagstepkeys: diagstepout.append("%s: %s" %  (ddk,eval('str(%s)' % ddk) ) )
		with open('csstepdata.txt','w') as ofh:
			ofh.write('\n'.join(diagstepout))
