...
if __name__=='__main__':
	selfuncfile='%s/select_model_fft18.py' % os.path.dirname(argv[0])
...
def tedpca(ste=0,mlepca=True): 
	...
	if not os.path.exists(pcastate_fn):
		...
		#Save state
		print "Saving PCA"
		pcastate = {'u':u,'s':s,'v':v,'ctb':ctb,'cAl':cAl,'eigelb':eigelb,'spmin':spmin,'spcum':spcum}
...
if __name__=='__main__':
	...
	#parser.add_option('',"--wav",action='store_true',dest='wav',help="Perform wavelet PCA, default False",default=False)
	...
	if options.mixm == None:
		...
		acc,rej,midk,empty = selcomps(seldict,knobargs=args,debug=debug_mode)
		del dd
	else:
