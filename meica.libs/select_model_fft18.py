...
def do_svm(train_set,train_labs,test_set,svmtype=0):
	...
	if svmtype==1: svm.LinearSVC(loss='squared_hinge',penalty='l1',dual=False)
...
def fft_variance(fproj_arr,fproj_arr_val,A,B):
	...
	fproj_sel_A = (andb([fproj_sel_T[0]>0,fproj_sel_T[1]<0.02])==2).reshape(mask.shape[0:2])
	fproj_sel_B = (andb([fproj_sel_T[0]<0,fproj_sel_T[1]<0.02])==2).reshape(mask.shape[0:2])
...
def selcomps(seldict,debug=False,olevel=2,oversion=99,knobargs='',filecsdata=False,savecsdiag=True):

	selmodelversion='fft18.072616'
	...
	tt_table[np.isinf(tt_table[:,0])]=np.percentile(tt_table[:,0],98)

	
	#Time series derivative kurtosis
	mmix_dt = (mmix[:-1]-mmix[1:])
	mmix_dt2 = (mmix_dt[:-1]-mmix_dt[1:])
	mmix_skew = stats.skew(mmix)	
	mmix_kurt = stats.kurtosis(mmix_dt2)
	...
	fw4m_z = []
	for ii in nc: 
		fproj = np.fft.fftshift(np.abs(np.fft.rfftn(unmask(seldict['tsoc_B'],mask)[:,:,:,ii])))
		fproj_z = fproj.max(2)
		...
		fw4m_z.append(np.max([ fitgaussian(fproj.max(jj))[3:].max() for jj in range(len(fprojr.shape)) ]))
	fw4m = np.array(fw4m_z)
	spr = np.array(spr)
	...
	fw4m[fw4m>np.median(fw4m)*5] = np.median(fw4m)*5
	...
	fz = np.array([spz,Tz,Vz,Ktz,KRr,cnz,Rz,mmix_kurt])
	...
	if len(group0)<2 or (len(group0)<4 and float(len(rej))/len(group0)>3):
		print "WARNING: Extremely limited BOLD signal space, reporting minimum selection as accepted."
		midkfailed = True
		diagstepkeys=['rej','KRcut','Kcut','Rcut','dbscanfailed','midkfailed','KRguess','group0']		
		diagstepout=[]
	...
	"""
	Step 2: Recompute tt_table to handle bias from high FR2 null distribution such as during tasks
	"""
	#recompute T based on what's identified as rej
	noisedist = np.array([])
	for ii in group0:	
		comp_noise_sel = andb([np.abs(Z_maps[:,ii])>1.95,Z_clmaps[:,ii]==0])==2
		noise_FR2_Z = np.log10(np.unique(F_R2_maps[unmask(comp_noise_sel,mask)[t2s!=0],ii]))
		noisedist = np.hstack([noisedist,noise_FR2_Z])
	for ii in nc:
		signal_FR2_Z  = np.log10(np.unique(F_R2_maps[unmask(Z_clmaps[:,ii],mask)[t2s!=0]==1,ii]))
		ttest = stats.ttest_ind(signal_FR2_Z,noisedist,equal_var=False)
		mwu = stats.norm.ppf(stats.mannwhitneyu(signal_FR2_Z,noisedist)[1])
		tt_table[ii,2] = np.abs(mwu)*ttest[0]/np.abs(ttest[0])
		tt_table[ii,3] = ttest[1]
	if debug:
		import ipdb
		ipdb.set_trace()
	realmask = andb([np.isinf(tt_table[:,2]),np.isnan(tt_table[:,2])])==0
	maxmask = andb([tt_table[:,2]>0,np.isinf(tt_table[:,2])])==2
	minmask = andb([~realmask,~maxmask])==2
	realt = tt_table[realmask,2]
	tt_table[maxmask,2] = np.max(realt)
	tt_table[minmask,2] = np.min(realt)
	...
	"""
	Step 3: Find spatial and temporal features that differentiate good and bad
	"""
	#Spatial FFT width	
	spz2 = (fw4m-fw4m[group0].mean())/fw4m[group0].std()
	#Spatial FFT features
	fproj_varA,fproj_varB = fft_variance(fproj_arr,fproj_arr_val,group0,rej)
	fproj_sel_v2 = fproj_varB/fproj_varA
	spz3 = (fproj_sel_v2-fproj_sel_v2[group0].mean())/fproj_sel_v2[group0].std()
	#Temporal features
	mmix_skew_z = (mmix_skew-mmix_skew[group0].mean())/mmix_skew[group0].std()
	mmix_kurt_z = (mmix_kurt-mmix_kurt[group0].mean())/mmix_kurt[group0].std()
	
	"""
	Step 4a: Classifiy midk and ignore using separte SVMs for difference variance regimes
	...
	"""
	#Update features
	tt_max = tt_table[:,[0,2]].max(1)
	tt_max[spz<0] = tt_table[spz<0,0]
	Tv_mean = np.average(np.array([tt_table[:,0],tt_max]).T,weights=np.array([np.max([1-spz,np.zeros(len(spz))],0),np.ones(len(spz))]).T,axis=1)
	Tz2 = (Tv_mean-Tv_mean[group0].mean())/Tv_mean[group0].std()
	Tz3 = (tt_table[:,0]-tt_table[group0,0].mean())/tt_table[group0,0].std()
	Vz2 = (varex_ - varex_[group0].mean())/varex_[group0].std()
	Vz3 = (varex - varex[group0].max())/varex[group0].std()
	Rz2 = (Rhos-Rhos[group0].mean())/Rhos[group0].std()
	Ktz2 = (Ktz - Ktz[group0].mean())/Ktz[group0].std()	

	if debug:
		import ipdb
		ipdb.set_trace()
	
	#Differentiate intermediate TE-dependence good components from high-frequency low TE-dependence artifacts, GRAPPA-like
	fz = np.array([spz2,Tz2,Tz3])		
	midk_clf_A,clf_ = do_svm(fz[:,np.union1d(group0,rej)].T,[0]*len(group0) + [1]*len(rej),fz[:,to_clf].T,svmtype=1)
	fz = np.array([spz2,spz3,Rz2])
	midk_clf_B,clf_ = do_svm(fz[:,np.union1d(group0,rej)].T,[0]*len(group0) + [1]*len(rej),fz[:,to_clf].T,svmtype=1)
	midk_clf = np.max([midk_clf_A,midk_clf_B],0)
	toacc_hi = to_clf[midk_clf==0]
	toacc_hi = np.setdiff1d(toacc_hi,toacc_hi[andb([fz[:,toacc_hi].max(0)>5,Vz3[toacc_hi]>10])>0])
	midkA = to_clf[midk_clf==1]
	midk = midkA[varex[midkA]>=np.median(varex[group0])]

	#Differentiate low variance TE-dependence good components from high-dimensional low variance space
	toacc_lo = np.intersect1d(to_clf,nc[andb([spz<1,spz2<4,Rz<1,mmix_kurt_z<5,Dz>-1,Tz>-1,Vz<0,Kappas>=F05])==8])

	"""
	Step 5: Compute variance associated with low T2* areas (e.g. draining veins and low T2* areas) 
	...
	"""
	Step 6: Learn joint TE-dependence spatial and temporal models to move remaining artifacts to ignore class
	"""
	if debug:
		import ipdb
		ipdb.set_trace()
	...
	field_art = np.setdiff1d(nc[andb([mmix_kurt_z>5,Kappas<minK_ign])==2],group0)
	field_art = np.union1d(np.setdiff1d(nc[andb([mmix_kurt_z>2,rankvec(mmix_kurt_z)-rankvec(Kappas)>newcest/2,Vz2>-1,Kappas<F01])==4],group0),field_art)
	...
	if savecsdiag:
		diagstepkeys=['selmodelversion','rej','KRcut','Kcut','Rcut','dbscanfailed','KRguess','group0','dice_rej','rej_supp','to_clf','midk','toacc_hi','toacc_lo','field_art','phys_art','misc_art','ncl','ign']
		diagstepout=[]
		for ddk in diagstepkeys: diagstepout.append("%s: %s" %  (ddk,eval('str(%s)' % ddk) ) )
		with open('csstepdata.txt','w') as ofh:
			ofh.write('\n'.join(diagstepout))
		allfz = np.array([spz,Tz,Vz,Ktz,KRr,cnz,Rz,spz2,Tz2,Dz])
